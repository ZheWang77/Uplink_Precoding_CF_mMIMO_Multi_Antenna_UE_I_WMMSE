function [coherentx,nonCoherentx] = functionMMSE_interferenceLevels( R_AP,M,K,N,tau_p,p,Pset)
...
for k = 1:K
    ...
    for l=1:K  %Non-coherent interference (i=k')
        ...
        nonCoherent(k,l) = p(l)*trace(Ksi(:,:,k,l));
        
        if any(l==Pset(:,k)) 
            
            coherent(k,l)=  p(l)*abs(trace(X_p1(:,:,k,l)))^2; 
