function [F_Precoding_Optimal] = functionOptimalObjective_LSFD(A,E,GG,Gkk,W,K,N,p)
%%=============================================================
%This function is used to generate I-WMMSE precoding matrices for the LSFD processing of the paper:
%
...
for k = 1:K
    ...
    F_power_up = norm((GG_total(:,:,k) + lambda_up*eye(N))\(W(k)*Gkk(:,:,k)'*A(:,:,k)/E(:,:,k)),'fro')^2 - p(k);
    ...
    if F_power_up < 0
        ...
        lambda = 0;
        F_Precoding_Optimal(:,:,k) = (GG_total(:,:,k) + lambda*eye(N))\(W(k)*Gkk(:,:,k)'*A(:,:,k)/E(:,:,k));
         
    else
 
        
    F_power_down = norm((GG_total(:,:,k) + lambda_down*eye(N))\(W(k)*Gkk(:,:,k)'*A(:,:,k)/E(:,:,k)),'fro')^2 - p(k);
    ...
    while(F_power_up * F_power_down < 0)
        ...
        F_power_now = norm((GG_total(:,:,k) + lambda*eye(N))\(W(k)*Gkk(:,:,k)'*A(:,:,k)/E(:,:,k)),'fro')^2 - p(k);
    ...
    F_Precoding_Optimal(:,:,k) = (GG_total(:,:,k) + lambda_optimal*eye(N))\(W(k)*Gkk(:,:,k)'*A(:,:,k)/E(:,:,k));
